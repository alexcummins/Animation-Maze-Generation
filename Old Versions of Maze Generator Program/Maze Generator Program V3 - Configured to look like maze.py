...
def drawG(mazeGrid):
    ...
    nx.draw(mazeGrid, pos=pos, edges=mazeGrid.edges(), edge_color="white", width=20, node_size=350, node_color="red")
    fig = plt.figure(1)
...
# -----------------------------------------------------------------------------------
    if algorithm == 'Growing Tree':
        mazeAttributes = {'newest': 25, 'oldest': 25, 'middle': 25, 'random': 25}  # MUST all add to 100
        currentNode = startNode
        ...
        for y in range(GRIDSIZE_Y):
            for x in range(GRIDSIZE_X):  # for every node (x, y) in the grid
                ...
                if chance == 0:
                    ...
                    elif (x, y + 1) in fullGrid.neighbors((x, y)):
                        mazeGrid.add_edge((x, y), (x, y + 1))
                if chance == 1:
                    if (x, y + 1) in fullGrid.neighbors((x, y)):
                        mazeGrid.add_edge((x, y), (x, y + 1))
                    elif (x + 1, y) in fullGrid.neighbors((x, y)):
                        mazeGrid.add_edge((x, y), (x + 1, y))
