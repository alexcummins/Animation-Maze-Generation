...
def getFigures(algorithm, mazeGrid, currentNode, newEdge, currentNodeList, currentIterNode):
    plt.clf()
    pos = dict([(n, n) for n in list(mazeGrid.nodes())])  # Makes every node have an attribute of it's own length
...
def createMaze(algorithm, newest, oldest, middle, rand):
    ...
    currentNode = None  # Failsafe if algorithm doesn't have these
    newEdge = None
    ...
    if algorithm == 'Growing Tree':
        ...
        while True:
            ...
            if possibleEdgeList == []:
                ...
                # if newest != 100:
                    # added for animation OR NOT
                    # for node in currentNodeList:
                    #     possibleEdgeList = getEdges(fullGrid, mazeGrid, visitedNodeList, node, startNode)
                    #     if possibleEdgeList == []:
